...
Clearly, `offset` and `delta` only apply to _regularly_ discretized
dimensions, and are `NA` if this is not the case. `from` and
`to` will usually be 1 and the dimension size, but `from` may
be larger than 1 in case a regular sub-grid got cut out or was
cropped. Rectilinear and curvilinear grids need grid values in
`values`; this can be irregularly _spaced_ coordinate values, or
coordinate _intervals_ of irregular width, or spatial geometries
encoded in an `sfc` vector ("list-column"), or a matrix with grid
cell centre values (longitude or latitude) for curvilinear grids.
